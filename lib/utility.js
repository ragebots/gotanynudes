...
const downloadImage = async (page, fileLink, filePath) => {
  return new Promise(async (resolve, reject) => {
    let alreadyResolved = false;
    page.on('response', async (response) => {
      if(response.url().includes(fileLink) && response.status() === 200) {
    ...
    try {
      await page.waitForSelector("#cf-bubbles", {state: "detached", timeout: 5 * 60 * 1000});
      await page.waitForNavigation({waitUntil: "networkidle",  timeout: 1 * 1000});
    } catch(err) {

    }
    if(!alreadyResolved) {
      reject("file link not loaded");
    }
